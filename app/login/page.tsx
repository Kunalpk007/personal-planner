...
import { signInWithPopup, signInWithRedirect, GoogleAuthProvider, getRedirectResult, signInWithEmailAndPassword } from 'firebase/auth'
...
async function exchangeToken(idToken: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 403) throw { code: 'auth/user-disabled' }
...
function LoginForm() {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      if (mounted.current) setSsoLoad(false)
    }
  }
